...
function asteroid_belt() {
	...
	for(var i = 0; i < 5; i++) {
		...
		var asteroid = { angle: aa, x: ax, y: ay, uid: uuid(), img: af, size: 1 + Math.random()};
...
function meteor_shower() {
	...
	var meteor = { angle: aa, x: ax, y: ay, uid: uuid(), frameid: 0, type: ty};
	meteors.push(meteor);
...
console.log(new Date().getTime(),asteroids_lastbelt, new Date().getTime() - asteroids_lastbelt)
...
function onFalconDeath() {
	...
	// var gamedata = JSON.parse(); //parseInt((JSON.parse(data)).d);	
